...
class FireStoreUtils {
  ...
  static Future<EmailTemplateModel?> getEmailTemplates(String type) async {
    ...
    await firestore.collection("emailTemplates").where(emailTemplates).where('type').get().then((value) {
      print("------>");
      if (value.docs.isNotEmpty) {
        print(value.docs.first.data());
  ...
  Future<List<OfferModel>> getOfferByVendorID(String vendorID) async {
    ...
    QuerySnapshot<Map<String, dynamic>> bannerHomeQuery =
        await firestore.collection(COUPON).where("resturant_id", isEqualTo: vendorID).where("isEnabled", isEqualTo: true).where("isPublic", isEqualTo: true).get();
  ...
  static sendOrderEmail({required OrderModel orderModel}) async {
    ...
    EmailTemplateModel? emailTemplateModel = await FireStoreUtils.getEmailTemplates(emailTemplates);
print("emailTemplateModel == ${emailTemplateModel!.id}");
    String newString = emailTemplateModel.message.toString();
    newString = newString.replaceAll("{username}", MyAppState.currentUser!.firstName + MyAppState.currentUser!.lastName);
